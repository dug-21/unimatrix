...
// NOTE (vnc-025, ADR-005): the PreCompact transcript extraction core
// (ExchangeTurn, build_exchange_pairs, format_turn, extract_transcript_block,
// prepend_transcript, truncate_utf8, MAX_PRECOMPACT_BYTES, TAIL_MULTIPLIER)
// moved to `uds/transcript_block.rs` so the server-side PreCompact path can
// share it. Call sites above re-import; behavior unchanged (R-14).
