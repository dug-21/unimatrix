...
async fn handle_connection(
    ...
    store: Arc<Store>,
    embed_service: Arc<EmbedServiceHandle>,
    // vnc-046 AC-09/FR-11: `dispatch_request` no longer takes a vector store or
    // adaptation service (vestigial params deleted). Still forwarded here from the
    // accept loop so the UDS wiring stays structurally unchanged (NFR-4); not used
    // past this point.
...
pub(crate) async fn dispatch_request(
    ...
    match request {
        ...
        HookRequest::ContextSearch {
            ...
            max_tokens: _,
            source,
            accept: _, // vnc-027 ADR-001: extracted pre-dispatch in handle_connection
        } => {
