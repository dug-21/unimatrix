...
// non-trivial CC@k, ICD, delta values and section 6 follows section 5.
// ---------------------------------------------------------------------------

#[test]
fn test_report_round_trip_cc_at_k_icd_fields_and_section_6() {
    // Build a synthetic ScenarioResult with non-zero, non-trivially-round values.
    let mut profiles = HashMap::new();
    profiles.insert(
        "baseline".to_string(),
        ProfileResult {
            ...
            p_at_k: 0.600,
            mrr: 0.500,
            cc_at_k: 0.714, // baseline cc_at_k: candidate - delta = 0.857 - 0.143
            icd: 0.857,     // baseline icd: candidate - delta = 1.234 - 0.377
        },
    );
    profiles.insert(
        "candidate".to_string(),
    ...
    // Serialize to JSON then deserialize — verifies backward compat (ADR-003).
    ...
    // Verify deserialized values round-tripped correctly.
    ...
    // Write deserialized result to disk and call run_report.
    let results_dir = TempDir::new().unwrap();
    let out_dir = TempDir::new().unwrap();
    let out_path = out_dir.path().join("report.md");
    ...
    // Assert non-trivial values appear in rendered output.
    assert!(
...
// ---------------------------------------------------------------------------
// nan-008 AC-13: All six sections present in correct order, CC@k and ICD in
// Summary, no section duplicated.
// ---------------------------------------------------------------------------
...
fn test_report_contains_all_six_sections() {
    ...
    // Two-profile scenario with non-zero CC@k and ICD values.
