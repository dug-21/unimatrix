...
// ---------------------------------------------------------------------------
// AC-08: Seven sections present in correct order (R-17, R-02, AC-12)
// ---------------------------------------------------------------------------

#[test]
fn test_report_contains_all_seven_sections() {
    let results_dir = TempDir::new().unwrap();
    let out_dir = TempDir::new().unwrap();
    let out_path = out_dir.path().join("report.md");

    // Use results with non-null phase so section 6 renders (required for AC-12).
    let mut r1 = make_scenario_result("scen-1", "query one", 0.8, 0.7, 0.85, 0.75);
    r1.phase = Some("delivery".to_string());
    let mut r2 = make_scenario_result("scen-2", "query two", 0.6, 0.5, 0.65, 0.55);
    r2.phase = Some("design".to_string());
    ...
    // Sections must appear in the order specified in FR-27 (positions 1-7).
    ...
    let pos6 = content.find("## 6. Phase-Stratified Metrics").unwrap();
    let pos7 = content.find("## 7. Distribution Analysis").unwrap();
    assert!(
        pos1 < pos2 && pos2 < pos3 && pos3 < pos4 && pos4 < pos5 && pos5 < pos6 && pos6 < pos7,
