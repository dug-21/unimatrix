...
impl Default for PendingEntriesAnalysis {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct UnimatrixServer {
    ...
    // rationale: live reads go through the service layer's own clone; this field is
    // retained for construction symmetry and direct-access parity with other stores.
    #[allow(dead_code)]
    ...
    // rationale: read only by the legacy `record_usage_for_entries`/`correct_with_audit`
    // methods (now superseded by the service layer); held for those test-exercised paths.
    #[allow(dead_code)]
    ...
    // rationale: stored at construction for tick wiring and test ptr-eq assertions; the
    // live readers are the service layer and background tick via their own clones.
    #[allow(dead_code)]
    ...
    // rationale: populated by the #[tool_router] macro for trait dispatch; not read
    // directly in production code (only via the generated routing and tests).
    #[allow(dead_code)]
...
impl UnimatrixServer {
    ...
    // rationale: server constructor wires together the full set of injected
    // collaborators; a params struct would mirror the same fields one-to-one.
    ...
    // rationale: legacy backend method superseded by the service-layer port; retained
    // with its test suite as the behavioral reference until the tests migrate.
    #[allow(dead_code)]
    ...
    // rationale: legacy backend method superseded by the service-layer usage port;
    // retained with its test suite as the behavioral reference until the tests migrate.
    #[allow(dead_code)]
...
pub(crate) mod tests {
    ...
    fn test_server_instructions_none_uses_compiled_default() {
        ...
        // rationale: the literal None is the subject under test — this asserts the
        // None-resolution path yields the compiled default verbatim.
    ...
    fn test_server_instructions_some_uses_config_string() {
        ...
        // rationale: the literal Some is the subject under test — this asserts the
        // Some(config_string) path is used verbatim, not the compiled default.
        #[allow(clippy::unnecessary_literal_unwrap)]
