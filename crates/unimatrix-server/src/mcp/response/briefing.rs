...
mod tests {
    ...
    fn format_index_table_empty_returns_empty_string() {
        ...
        assert!(
            result.is_empty(),
            "empty slice must return empty string, got: {:?}",
            result
        );
    }
    ...
    fn format_index_table_columns_present() {
        ...
        assert!(
            result.contains("0.60"),
            "confidence must be formatted as '0.60'"
        );
        assert!(
            !result.contains("0.6\n"),
        ...
        assert!(
            !result.contains("0.600"),
            "confidence must be '0.60' not '0.600'"
        );
        assert!(
        ...
        assert!(
            result.contains(" 1"),
            "row number must be right-justified (' 1')"
        );
    }
    ...
    fn snippet_truncation_ascii_exactly_150() {
        ...
        assert_eq!(
            snippet, content,
            "exactly 150 ASCII chars must not be truncated"
        );
    }
    ...
    fn snippet_truncation_longer_than_150() {
        ...
        assert_eq!(
            snippet.chars().count(),
            150,
            "300-char content must be truncated to 150"
        );
    }
    ...
    fn format_index_table_multiple_entries_numbered_sequentially() {
        ...
        assert!(
            lines[4].trim_start().starts_with('3'),
            "third data row starts with 3"
        );
    }
    ...
    fn format_index_table_confidence_formatted_consistently() {
        ...
        assert!(
            result.contains("1.00"),
            "confidence 1.0 must render as '1.00'"
        );
        assert!(
            result.contains("0.00"),
            "confidence 0.0 must render as '0.00'"
        );
    }
