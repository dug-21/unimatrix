...
// Rule 6 "dependency_on_deprecated" (vnc-015) was retired in bugfix-891 (#891): its input —
// Prerequisite edges touching a Deprecated entry — is deleted by the EveryTick orphaned-edge
// compaction within ~1 tick, but it only read at on-demand context_cycle_review time, so it was
// structurally starved and never fired. The detect-at-deprecate governance signal is tracked in #895.
