...
// Updated to 32 for vnc-049 (observations.source_domain + observations.model_id).
#[tokio::test]
async fn test_schema_version_is_32() {
    let dir = tempfile::TempDir::new().unwrap();
    let store = open_test_store(&dir).await;
    let version = store.read_counter("schema_version").await.unwrap();
    assert_eq!(
        version, 32,
        "schema version must be 32 after vnc-049 (was 31 after vnc-047)"
    );
    store.close().await.unwrap();
}
