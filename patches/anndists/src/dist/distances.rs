...
impl Distance<f32> for DistL1 {
    ...
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
        {
            if is_x86_feature_detected!("avx2") {
                return distance_l1_f32_simdeez(va, vb);
            }
        }
        assert_eq!(va.len(), vb.len());
    ...
    #[cfg(all(feature = "stdsimd", not(feature = "simdeez_f")))]
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        distance_l1_f32_simd(va, vb)
    }

    #[cfg(not(any(feature = "simdeez_f", feature = "stdsimd")))]
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        assert_eq!(va.len(), vb.len());
        va.iter().zip(vb.iter()).map(|t| (*t.0 - *t.1).abs()).sum()
    }
...
impl Distance<f32> for DistL2 {
    ...
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        ...
        {
            if is_x86_feature_detected!("avx2") {
                return distance_l2_f32_simdeez(va, vb);
            }
        }
        scalar_l2_f32(va, vb)
    }
...
fn scalar_dot_f32(va: &[f32], vb: &[f32]) -> f32 {
    let dot = 1.
        - va.iter()
            ...
            .map(|t| (*t.0 * *t.1))
            .fold(0., |acc, t| (acc + t));
    // Clamp to 0.0: floating-point rounding can produce slightly negative
    // values when vectors are nearly identical (dot product > 1.0).
    dot.max(0.)
}
...
impl Distance<f32> for DistDot {
    ...
    fn eval(&self, va: &[f32], vb: &[f32]) -> f32 {
        ...
        {
            if is_x86_feature_detected!("avx2") {
                return distance_dot_f32_simdeez(va, vb);
