...
assertion. The additional **R-MPII (#859)** two-filter marker-contract class (PII
...
| AC-04 | PENDING-operational | COVERED-BY-AC-11 (cold-model dispatch run). Pre-merge: PRESENT proven load-bearing via the real **MCP `context_store`** durable round trip by `test_warmup_present_requires_durable_read_roundtrip` (observe-durability would have zero embed dependency — corrected in `543e8d08`) |
