...
7. **Delivery gate R-01 (EXPLAIN QUERY PLAN)** — Before closing, run `EXPLAIN QUERY PLAN` on the back-fill SQL against a real schema. Confirm the NOT EXISTS sub-select uses the UNIQUE B-tree index (`sqlite_autoindex_graph_edges_1`), not a full scan. If a full scan appears, add a composite index to the v18→v19 migration DDL. Document the output as a comment in `migration_v18_to_v19.rs`. See GATE-3B-03 in RISK-TEST-STRATEGY.md.
8. **Delivery gate R-02 ("no duplicate" grep — non-negotiable)** — After updating tick tests, grep `co_access_promotion_tick_tests.rs` for the literal string `"no duplicate"`. It must return zero matches. `test_existing_edge_stale_weight_updated` (T-BLR-08) was written with explicit one-directional intent — it is not an accidental assertion. A passing test suite that still contains `"no duplicate"` asserts the wrong behavior. See GATE-3B-01 in RISK-TEST-STRATEGY.md.
9. **Delivery gate R-08 (even-only edge counts — non-negotiable)** — After updating blast-radius tests, grep for all `count_co_access_edges` and `assert_eq!(count` values. Every post-crt-035 count assertion must be even (0, 2, 4, 6, 10…). An odd value indicates a missed T-BLR update. The 8 affected tests (T-BLR-01 through T-BLR-08) are enumerated in SPECIFICATION.md §Test Blast Radius. See GATE-3B-02 in RISK-TEST-STRATEGY.md.
