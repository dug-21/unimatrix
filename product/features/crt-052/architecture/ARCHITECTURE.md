...
| C9 | **Config knobs** | extends `RetentionConfig` (`infra/config.rs`) | `transcript_candidate_session_cap_bytes` (default 24 KB), `transcript_candidate_cycle_cap_bytes`, `transcript_fallback_hole_fraction: f64` (default 0.5 — ADR-006 fallback trigger, ratified as a config knob not a compile-time constant, Gate 3a), `transcript_hold_max_sessions`, `transcript_hold_ttl_secs`. Same `serde(default)`/`validate()`/merge pattern as `transcript_buffer_max_bytes`. | A/B |
...
| **Candidate (C4)** | `pub struct TranscriptCandidate { pub session_id: String, pub byte_offset: u64, pub ts: Option<String>, pub family_hints: Vec<FamilyHint>, pub text: String }` — `Debug` MAY show `text` (it is response content the agent consumes; the R-19 metadata-only-Debug rule targets `TranscriptSnapshot`/`HoleInfo`/held-buffer types, not this response value — Gate 3a ratification, ADR-007) | NEW `unimatrix-observe/src/types.rs` |
...
| **Per-session loss (C4)** | `pub struct SessionLossInfo { pub session_id: String, pub elided_bytes: u64, pub has_holes: bool, pub provenance: CandidateProvenance, pub dropped_candidates: u64 }` — `dropped_candidates` is a content-free cap-forced-truncation count (per-session OR per-cycle aggregate cap), required to satisfy AC-08 no-silent-cap-drop (Gate 3a ratification, ADR-007) | NEW `types.rs` |
...
| **Held store (C8)** | `transcript_hold.rs`: `hold_on_drain(session_id, Arc<Mutex<TranscriptBuffer>>, feature_cycle)`, `readopt(session_id, registering_feature_cycle) -> Option<Arc<…>>`, `sweep_expired(ttl) -> Vec<TranscriptPurgeRecord>`, `purge_held_for_feature(feature_cycle) -> Vec<TranscriptPurgeRecord>` — both `hold_on_drain` (3-arg) and `readopt` (2-arg) carry `feature_cycle`; the re-registering cycle is mandatory for SR-02/AC-11/R-01 loud re-adoption on cycle MATCH (Gate 3a ratification, ADR-008) | NEW `infra/transcript_hold.rs` |
