...
| **AC-22 (clock/unit consistency — integration test)** | All timestamps entering the `compaction_reread` gate are normalized to Unix seconds (PostToolUse read `ts` in epoch millis floored to seconds via integer `ts_millis ÷ 1000`) before comparison, and a read counts iff `read_ts_secs > compacted_at` (STRICT). Worked example with `compacted_at = T` (Unix seconds): a read at **+1s** (`ts_millis = (T+1)·1000`) floors to `T+1`, and `T+1 > T` is true → it **COUNTS**; a read at **−500ms** (`ts_millis = (T−1)·1000 + 500`) floors to `T−1`, and `T−1 > T` is false → it does **NOT** count; a read exactly at the boundary (`ts_millis = T·1000`) floors to `T`, and `T > T` is false → it does **NOT** count (strict `>`). Expected `compaction_reread_count` for this single counting read = **1**. The −500ms case is the floor-catching guard: an implementation that forgot to normalize (comparing the raw millis `ts` ≈ `(T−1)·1000 ≫ T` directly against the seconds `compacted_at = T`) would wrongly count it; correct seconds-normalization rejects it — catching the ~1000× millis-vs-seconds mis-compare. | FR-14b | INTEGRATION test: seed a `compaction_events.compacted_at = T` (seconds) and PostToolUse overlapping reads: a read whose `ts_millis = (T+1)·1000` (+1s, floors to `T+1`) MUST count; a read whose `ts_millis = (T−1)·1000 + 500` (−500ms, floors to `T−1`) MUST NOT count; a read whose `ts_millis = T·1000` (exactly at the boundary, floors to `T`) MUST NOT count (strict `>`). Assert `compaction_reread_count == 1`. The −500ms case is the unit-mismatch guard: an unnormalized gate comparing raw millis `ts` against the seconds `compacted_at` would see `(T−1)·1000 ≫ T` and wrongly count it; assert that seconds-normalization (`ts_millis ÷ 1000` floor) prevents the ~1000× mis-compare so the gate comparison is unit-consistent. |
