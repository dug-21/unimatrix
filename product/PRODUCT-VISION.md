...
| SQLite Cutover | `nxs-006` | Migrate production data from redb to SQLite and make SQLite the sole backend. **Data migration**: Build export/import subcommands on `unimatrix-server` — the redb-compiled binary exports all 15 tables to a single intermediate file, the sqlite-compiled binary imports it. One production database to migrate. **Cutover**: After validated migration, remove redb backend, feature flags, and transitional compat layer. SQLite becomes the sole unconditional backend. `rusqlite` moves from optional to unconditional dependency. `redb` removed from workspace. **Scope**: unimatrix-store (remove redb impl ~6,200 lines, flatten sqlite/ module) + dependency cleanup in server/workspace Cargo.toml. No schema normalization — that's nxs-007. See `product/features/nxs-006/SCOPE.md`. |
| Schema Normalization & Server Decoupling | `nxs-007` | Two concerns, one feature — both require SQLite-only (nxs-006 complete). **Server decoupling**: Refactor `unimatrix-server` to use only the `EntryStore` trait API from `unimatrix-core`. Eliminate all direct imports of backend-specific transaction types, table definitions, and handle patterns (~8 server files). Move uncovered operations into new trait methods. Result: server has zero imports from `unimatrix-store` backend modules. **Schema normalization**: Exploit SQLite capabilities to simplify the data model. **(a) Full column decomposition**: Decompose ENTRIES bincode blob into 24 SQL columns (Option B from ASS-016). entry_tags junction table replaces TAG_INDEX. **(b) Index table elimination**: Replace 5 manual index tables with SQL CREATE INDEX statements. Eliminates ~300 lines of index synchronization in write.rs. **(c) Operational table normalization**: Decompose CO_ACCESS, SESSIONS, INJECTION_LOG, SIGNAL_QUEUE from bincode blobs to SQL columns. Add `session_id` column to INJECTION_LOG. **(d) Query simplification**: Replace client-side HashSet intersection with SQL WHERE clauses and JOINs. **Prerequisite for retrospective analytics**: Entry effectiveness scoring requires session_id column and SQL JOIN capability. **Prerequisite**: nxs-006 (SQLite is sole backend). See GH #97. |
...
**Ships**: Functional storage + retrieval backend. SQLite engine swap (nxs-005, complete) enables the analytics query patterns needed for retrospective evolution. SQLite cutover (nxs-006) migrates production data and removes redb. Schema normalization & server decoupling (nxs-007) eliminates manual index maintenance, enables SQL JOINs for structured event analysis, and ensures clean architecture for future storage evolution.
...
M1: Foundation (nxs)         ✅ COMPLETE (nxs-001/002/003/004)
 ...
 │    └─► nxs-006: SQLite Cutover  ← migrate prod DB, remove redb
 │         └─► nxs-007: Schema Normalization + Server Decoupling  ← clean architecture (#97)
 │
