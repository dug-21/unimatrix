...
| SQLite Storage Engine | `nxs-005` | Replace redb with SQLite (rusqlite, `bundled` feature) as the storage backend. **Zero functional change** — all 10 MCP tools return identical results, all 17 tables preserved as SQLite tables with equivalent schemas, bincode blob serialization unchanged. HNSW vector index remains in-memory with VECTOR_MAP bridge table in SQLite (same architecture, different storage engine — see Vector Architecture note below). **Migration strategy**: Dual-backend parity testing — implement SqliteStore alongside existing redb Store, run all 234 store tests against both backends to verify identical behavior before cutover. Data migration tooling (redb → SQLite export/import with row count verification). **Scope**: unimatrix-store crate only — db.rs, write.rs, read.rs, migration.rs, counter.rs rewrite (~5,500 lines, 70-85% of the crate). unimatrix-core (StoreAdapter), unimatrix-engine, unimatrix-server, unimatrix-vector require **zero changes** — the EntryStore trait boundary holds completely. Schema v5 migration chain preserved (v0→v5 with entry rewriting for v0→v3, table creation for v3→v5). **What NOT to change**: Do not normalize schema (keep blobs), do not replace index tables with CREATE INDEX, do not change bincode serialization, do not replace HNSW with sqlite-vec — each is a separate, testable follow-up. **Why now**: (1) Retrospective analytics evolution (col-002 structured events path) requires multi-table correlation queries (SESSIONS × INJECTION_LOG × ENTRIES for entry effectiveness scoring) that are trivial in SQL but require full table scans and client-side joins in KV. (2) Manual secondary indexes (5 of 17 tables) are ~300 lines of index synchronization code. (3) Co-access reverse lookup requires full table scan. (4) Schema migrations require scan-and-rewrite — SQL uses ALTER TABLE. See `product/research/ass-016/`. |
| Schema Normalization | `nxs-006` | Exploit SQLite capabilities to simplify the data model after nxs-005 stabilizes. Three changes: **(1) Index table elimination**: Replace 5 manual index tables (TOPIC_INDEX, CATEGORY_INDEX, TAG_INDEX, TIME_INDEX, STATUS_INDEX) with denormalized columns on the entries table + SQL CREATE INDEX statements. Eliminates ~300 lines of index synchronization in write.rs and the entire index-diff-on-update code path. **(2) Reverse lookup indexes**: Add `idx_co_access_b` index on co_access(entry_id_b) for O(1) reverse co-access lookup (currently full table scan). Add `session_id TEXT` column to injection_log for indexed cascade delete during session GC (currently deserialize-and-filter). **(3) Query simplification**: Replace client-side HashSet intersection for multi-filter queries with SQL WHERE clauses and JOINs. Tag intersection becomes `SELECT entry_id FROM tag_index WHERE tag IN (?) GROUP BY entry_id HAVING COUNT(*) = ?`. **Prerequisite for retrospective analytics**: Entry effectiveness scoring (`SELECT ... FROM injection_log il JOIN sessions s ON il.session_id = s.session_id JOIN entries e ON il.entry_id = e.id WHERE s.feature_cycle = ? GROUP BY e.id`) requires the session_id column and SQL JOIN capability. **Scope**: unimatrix-store only. No changes to trait signatures or consumers. |

**Status (2026-03-04):** nxs-001/002/003/004 ✅ COMPLETE. nxs-005/006 scoped — see ASS-016 research.

**Ships**: Functional storage + retrieval backend. SQLite engine swap (nxs-005) enables the analytics query patterns needed for retrospective evolution. Schema normalization (nxs-006) eliminates manual index maintenance and enables SQL JOINs for structured event analysis.
...
M1: Foundation (nxs)         ✅ COMPLETE (nxs-001/002/003/004)
 ├─► nxs-005: SQLite Storage Engine  ← parallel track (unimatrix-store only)
 │    └─► nxs-006: Schema Normalization  ← enables retrospective analytics
 │
