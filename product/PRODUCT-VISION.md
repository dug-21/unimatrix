...
| Schema Normalization & Cutover | `nxs-006` | Complete the SQLite migration and normalize the schema. **Phase 1 — Data Migration & Cutover**: Build export/import subcommands on `unimatrix-server` — the redb-compiled binary exports all 15 tables to a single intermediate file, the sqlite-compiled binary imports it. One production database to migrate. After validated migration, remove redb backend, feature flags, and transitional compat layer. SQLite becomes the sole unconditional backend. **Phase 2 — Schema Normalization**: Exploit SQLite capabilities to simplify the data model. **(a) Index table elimination**: Replace 5 manual index tables (TOPIC_INDEX, CATEGORY_INDEX, TAG_INDEX, TIME_INDEX, STATUS_INDEX) with SQL columns on entries table + CREATE INDEX statements. Eliminates ~300 lines of index synchronization in write.rs. **(b) Full column decomposition**: Decompose ENTRIES bincode blob into 24 SQL columns (Option B from ASS-016). entry_tags junction table replaces TAG_INDEX. **(c) Operational table normalization**: Decompose CO_ACCESS, SESSIONS, INJECTION_LOG, SIGNAL_QUEUE from bincode blobs to SQL columns. Add `session_id` column to INJECTION_LOG for indexed cascade deletes. **(d) Query simplification**: Replace client-side HashSet intersection for multi-filter queries with SQL WHERE clauses and JOINs. **Prerequisite for retrospective analytics**: Entry effectiveness scoring requires session_id column and SQL JOIN capability. **Scope**: unimatrix-store only + dependency cleanup in server/workspace Cargo.toml. No changes to public Store API signatures. See `product/features/nxs-006/SCOPE.md` and `product/research/ass-016/`. |
| Server Storage Decoupling | `nxs-007` | Refactor `unimatrix-server` to use **only** the `EntryStore` trait API from `unimatrix-core`. Eliminate all direct imports of backend-specific transaction types, table definitions, and handle patterns. Currently ~8 server files bypass the Store API to directly use redb/sqlite transaction internals (discovered during nxs-005, addressed with transitional compat layer). Move any operations not covered by `EntryStore` into new trait methods. Result: server has zero imports from `unimatrix-store` backend modules, adding a third backend (e.g., PostgreSQL) requires zero server changes, server can be tested with mock stores. **Prerequisite**: nxs-006 (SQLite is sole backend). See GH #97. |

**Status (2026-03-05):** nxs-001/002/003/004/005 ✅ COMPLETE. nxs-006 scoped (migration + normalization), nxs-007 scoped (server decoupling, #97).

**Ships**: Functional storage + retrieval backend. SQLite engine swap (nxs-005, complete) enables the analytics query patterns needed for retrospective evolution. Data migration & schema normalization (nxs-006) completes the cutover and eliminates manual index maintenance, enabling SQL JOINs for structured event analysis. Server decoupling (nxs-007) removes the transitional compat layer and ensures clean architecture for future storage evolution.
...
M1: Foundation (nxs)         ✅ COMPLETE (nxs-001/002/003/004)
 ├─► nxs-005: SQLite Storage Engine  ✅ COMPLETE (dual-backend, feature-flagged)
 │    └─► nxs-006: Migration & Schema Normalization  ← cutover + normalize
 │         └─► nxs-007: Server Storage Decoupling  ← clean architecture (#97)
 │
...
**nxs-005 completion & path forward (2026-03-05):** nxs-005 shipped with dual-backend feature flag (PR #96). Discovery during Gate 3c: unimatrix-server bypasses the Store API in ~8 files, directly coupling to redb transaction internals (214 compile errors under `backend-sqlite`). Addressed with a transitional compatibility layer (cfg-gated imports). All tests pass: 1,695 redb workspace, 1,548 SQLite workspace, 167 infra-001 integration. **Three-feature path**: nxs-005 (done) → nxs-006 (migrate production data via export/import subcommands, remove redb, normalize schema) → nxs-007 (remove transitional compat layer, server uses only EntryStore trait). **Migration approach**: The feature flag is compile-time, so the migration tool is built into unimatrix-server as export/import subcommands — the redb binary exports all 15 tables to a single file, the sqlite binary imports it. One production database to migrate. nxs-006 SCOPE.md updated to include migration tooling and redb removal alongside schema normalization.
