...
|-------|------|------------|
| `uni-scrum-master` | **Coordinator** — reads protocol, spawns workers, manages gates | Primary agent |
| `uni-validator` | **Validation gate** — spawned at each gate with focused checks | Scrum-master |
...
| `uni-validator` | gate | Three-gate validator. Spawned with different check sets at Gates 3a, 3b, 3c. Reports PASS / REWORKABLE FAIL / SCOPE FAIL |
...
1. **Every swarm**: uni-scrum-master (coordinator) + uni-validator (gate). No exceptions.
...
4. **Domain specialists**: As Unimatrix domain agents are created, they can be added to Stage 3b alongside uni-rust-dev.
...
1. Coordinator (uni-scrum-master) spawns agents via `Task` tool
