...
| `context_status` | Get knowledge base health metrics. Shows entry counts, distributions, correction chains, coherence score, security metrics, and graph cohesion metrics (connectivity rate, isolated entry count, cross-category edge count, Supports edge count, mean entry degree, and NLI-inferred edge count). **Admin only.** | When you need to assess knowledge base health or inspect whether NLI-based edge inference is producing a connected, cross-category graph. The `maintain` parameter is accepted but silently ignored — a background tick handles maintenance automatically. Key params: `topic`, `category`, `check_embeddings`. |
...
SQLite local database (`unimatrix.db`). 20 tables. Schema version 16. Zero cloud dependency — all data stays on your machine.
...
~/.unimatrix/{project-hash}/
  ...
  unimatrix.db               # SQLite knowledge database (schema v16)
