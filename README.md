...
Unimatrix exposes 14 MCP tools. All tools accept `format: "summary" | "markdown" | "json"` as a common parameter — except `context_cycle_review`, which accepts only `"markdown" | "json"` (the former `"summary"` alias is no longer accepted and returns an invalid-params error).
