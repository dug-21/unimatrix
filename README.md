...
Provider identity (`"claude-code"`, `"gemini-cli"`, `"codex-cli"`, `"opencode"`) is derived from the `--provider` flag or inferred from the hook event name at the ingest boundary and carried through as `source_domain` on observation records. Gemini CLI events with unique names (`BeforeTool`, `AfterTool`, `SessionEnd`) are unambiguously identified without a flag; Codex CLI requires `--provider codex-cli` because it shares event names with Claude Code. OpenCode events pass `--provider opencode` from the plugin shim and resolve to an explicit `opencode` `source_domain`, never the hook-path `claude-code` default. OpenCode events may additionally carry the backend model identity via `unimatrix hook --model <providerID/modelID>` (e.g. `ollama/qwen3-coder`); this `model_id` is persisted at ingest so local-model activity is queryable as distinct from cloud-model activity.
...
| `hook <EVENT>` | Handle a lifecycle hook event from Claude Code, Gemini CLI, Codex CLI, or OpenCode. Reads JSON from stdin, connects to the running server via UDS. Provider-specific event names (e.g., Gemini's `BeforeTool`, `AfterTool`, `SessionEnd`) are normalized to canonical Unimatrix names at the ingest boundary. OpenCode events are delivered by its in-process TS plugin with `--provider opencode`. Designed for use in hook configuration files, not direct user invocation. | Event name as positional arg. `--provider <name>` (`claude-code` \| `gemini-cli` \| `codex-cli` \| `opencode`) — required for Codex (shares event names with Claude Code) and OpenCode; optional for Gemini (inferred from event name); omit for Claude Code (backward-compatible default). `--model <providerID/modelID>` (e.g. `ollama/qwen3-coder`) — optional backend-model identity, persisted as `model_id` for local-vs-cloud attribution; an invalid or absent value yields none. |
