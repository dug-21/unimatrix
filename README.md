...
| `context_correct` | Correct an existing entry. Deprecates the original and creates a new entry with a hash-chain link. Automatically redirects all incoming graph edges (excluding `Supersedes`) from the deprecated original to the new entry — no separate `context_edge(mode="redirect")` call required. Response text includes `"Redirected N incoming edges (M failed, see logs)"` when edges are affected. | When an entry contains wrong or outdated information that should be superseded (not just hidden). Key params: `original_id` (required), `content` (required), `reason`, `edges` (optional list of `{edge_type, target_id}` — edges attach to the new corrected entry, not the deprecated original). |
...
| `context_graph` | Graph read operations on the knowledge graph. Three modes: `chain` walks the full supersession history of an entry (ancestors and descendants) via recursive SQL CTE; `current` follows the `superseded_by` chain to the terminal active entry; `neighbors` retrieves entries connected by typed edges at configurable depth. All three modes require `Capability::Read`. Safety cap: 50 hops for chain and current traversal; depth 1–10 for neighbors. `neighbors` at depth=1 queries the live database (all committed writes reflected immediately); depth>1 queries the in-memory graph (may lag recent writes by up to one tick interval). | When you need to navigate the knowledge graph without semantic search. Use `chain` to audit correction history; use `current` to resolve a deprecated entry to its live successor; use `neighbors` to retrieve entries related by typed edges (e.g., all `Prerequisite` dependencies of a decision, all `Supports` edges pointing at a claim). Key params: `mode` (required: `"chain"` \| `"current"` \| `"neighbors"`), `id` (required for all three modes), `direction` (`"forward"` \| `"backward"` \| `"both"`, chain and neighbors), `edge_types` (list of `RelationType` strings; absent or empty = all types excluding `Supersedes`), `depth` (neighbors, default 1, range 1–10), `resolve_supersessions` (neighbors only, default false — substitutes deprecated neighbor endpoints with their terminal active successors). |
